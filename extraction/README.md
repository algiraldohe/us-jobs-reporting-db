...
Service designed for retrieving Daily Job Opportunity Announcements data for a given keyword spaning a time frame and an determined location .
...
This behaviour needs to be validated with support from the API or derive a functional business logic with the product owner or main stakeholder in order to create a workaround and understand the implications and limitations of this behaviour further. For the purpose of the exercise, I continue to the development with the certainty that thanks to the design any change-requirement can be easily implemented in terms of the `search` service.
...
- Create a more robust validation in the console adapter to be able to catch typos or map intended parameters, guiding the user in mistakes that might be made.
...
- In `extraction.domain` the `extract_jobs()` function should implement pagination for when the output results are larger than a page can manage, and still ensure that all the data that matches the search is retrieved. Additionally, proper batching operations to store the data efficiently through files and not end up with the whole data loaded in memory and then dumped into a huge file that will be very problematic to read or work with after.

- More robust exception and errors handling throughout the process to ensure proper feedback to the user and guarantee that any bugs that the application may present are caught properly.

- Familiarise better with the API in order to handle better the different `status.code` that the service may receive when performing the search, allowing proper bifurcation and convergence of what the issue is and how to solve it.
